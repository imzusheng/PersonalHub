...
interface StatusResponse { mode:string; connector:string; agentStatus:string; apiHost:string; apiPort:number; lastHeartbeatAt:string|null; lastTick:TickResult|null; pluginCount:number; capabilityCount:number; startedAt:string; hostId:string|null; memoryPercent:number; taskCount:number; platform:string }
...
async function render():Promise<void>{
  ...
  try{
    ...
    const running=status.agentStatus==='running';
    app.innerHTML=`<div class="app-shell ${status.platform==='darwin'?'mac-immersive':''}"><div class="app-frame">
      <header class="topbar glass"><div class="brand"><div class="brand-mark">P</div><div><strong>PersonalHub</strong><small>Local AI Worker</small></div></div><nav class="nav">${nav()}</nav><div class="top-actions"><span class="connection ${running?'online':'offline'}">${running?'远程调度中':'远程调度已停止'}</span><button class="tool-btn" id="topRefresh">刷新</button></div></header>
...
async function renderTab(status:StatusResponse):Promise<string>{
  ...
  if(activeTab==='plugins'){
    ...
    const body=plugins.length?`<div class="grid plugin-grid">${plugins.map((plugin)=>card(`<div class="task-head"><div class="icon-box">${esc(plugin.name.slice(0,1).toUpperCase())}</div><span class="badge ${plugin.enabled?'ok':'warn'}">${plugin.enabled?'已启用':'已关闭'}</span><button class="tool-btn ${plugin.enabled?'warn':'ok'} icon-only toggle-plugin" data-plugin-id="${esc(plugin.id)}" data-action="${plugin.enabled?'disable':'enable'}" title="${plugin.enabled?'关闭插件':'启用插件'}">${plugin.enabled?'⏸':'▶'}</button><button class="tool-btn danger icon-only delete-plugin" data-plugin-id="${esc(plugin.id)}" title="注销插件">✕</button></div><h3>${esc(plugin.name)}</h3><p>${esc(plugin.description??`${plugin.runtime} · ${plugin.version}`)}</p><div class="chips">${plugin.capabilities.map((cap)=>`<span class="chip">${esc(cap.name)}</span>`).join('')}</div><div class="mono" style="margin-top:16px">${esc(plugin.id)} · ${esc(plugin.runtime)}</div>`,'plugin-card')).join('')}</div>`:`<div class="empty"><div><strong>尚未安装插件</strong><span>把插件放入 PersonalHub 数据目录后重启应用</span></div></div>`;
  ...
  return pageHead('Overview','运行概览','本机 AI 服务、远程调度与执行状态',hostMeta)+`
    ...
    <div class="grid split-grid">
      ${card(`<div class="section-title"><h3>调度状态</h3><span>Control Plane</span></div><div class="summary"><div><span>最近同步</span><strong>${esc(fmtTime(status.lastHeartbeatAt))}</strong></div><div><span>本次处理</span><strong>${tick?.tasksProcessed??0}</strong></div><div><span>运行时长</span><strong>${esc(elapsed(status.startedAt))}</strong></div></div><div class="toolbar" style="margin-top:16px"><button class="tool-btn ${running?'danger':'primary'}" id="toggleScheduling">${running?'停止远程调度':'启动远程调度'}</button><button class="tool-btn" id="runTick" ${running?'':'disabled'}>立即同步</button><button class="tool-btn" id="checkUpdate">检查更新</button></div><pre class="result" id="actionOutput"></pre>`)}
      ${card(`<div class="section-title"><h3>运行边界</h3><span>Local First</span></div><div class="list"><div class="list-row"><div class="icon-box">L</div><div><strong>本地插件继续运行</strong><small>停止调度不会停止 Ollama 或删除插件</small></div><span class="badge ok">独立</span></div><div class="list-row"><div class="icon-box">A</div><div><strong>adminOS 主动连接</strong><small>客户端主动心跳、同步与领取任务</small></div><span class="badge ${running?'ok':'warn'}">${running?'在线':'暂停'}</span></div><div class="list-row"><div class="icon-box">U</div><div><strong>应用更新</strong><small>安装包校验后由用户确认安装</small></div><span class="badge">手动</span></div></div>`)}
    </div>`;
}
...
function bindEvents():void{
  ...
  document.addEventListener('click',(event)=>{const target=(event.target as HTMLElement).closest('.delete-plugin') as HTMLButtonElement|null;if(!target||target.disabled)return;const pluginId=target.dataset.pluginId;if(!pluginId)return;void runAction(target,async()=>{const result=await window.personalhub!.unregisterPlugin(pluginId);if(result?.ok){toast(`插件 ${pluginId} 已注销`);await render()}else{toast(result?.error??`"注销失败"`)}})});
  document.addEventListener('click',(event)=>{const target=(event.target as HTMLElement).closest('.toggle-plugin') as HTMLButtonElement|null;if(!target||target.disabled)return;const pluginId=target.dataset.pluginId;const action=target.dataset.action;if(!pluginId||!action)return;void runAction(target,async()=>{const result=await window.personalhub![action==='enable'?'enablePlugin':'disablePlugin'](pluginId);if(result?.ok){toast(`插件 ${pluginId} ${action==='enable'?'已启用':'已关闭'}`);await render()}else{toast(result?.error??`"操作失败"`)}})});
